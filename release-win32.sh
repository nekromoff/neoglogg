...
if [ -z "$VERSION" ]; then
    ...
    qmake neoglogg.pro -spec win32-g++ -r CONFIG+=release BOOST_PATH=$BOOSTDIR
else
    echo "Building version $VERSION"
    qmake neoglogg.pro -spec win32-g++ -r CONFIG+=release BOOST_PATH=$BOOSTDIR VERSION="$VERSION"
fi
mingw32-make
...
/cygdrive/c/Program\ Files/NSIS/makensis -DVERSION=$VERSION neoglogg.nsi
